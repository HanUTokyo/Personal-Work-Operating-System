...
public class GlobalAiSuggestionRepository {
    ...
    public List<GlobalAiSuggestion> findAll(Long ownerUserId) {
        return jdbcTemplate.query("""
                SELECT id, content, created_at, updated_at
                FROM global_ai_suggestions
                WHERE owner_user_id = ? AND is_deleted = 0
                ORDER BY updated_at DESC, id DESC
                """, rowMapper, ownerUserId);
    }

    public GlobalAiSuggestion save(Long ownerUserId, String content, LocalDateTime now) {
        jdbcTemplate.update("""
                INSERT INTO global_ai_suggestions (owner_user_id, content, created_at, updated_at)
                VALUES (?, ?, ?, ?)
                """, ownerUserId, normalizeContent(content), formatDateTime(now), formatDateTime(now));
        return jdbcTemplate.query("""
                SELECT id, content, created_at, updated_at
                FROM global_ai_suggestions
                WHERE owner_user_id = ? AND is_deleted = 0
    ...
    public Optional<GlobalAiSuggestion> findById(Long ownerUserId, Long suggestionId) {
        List<GlobalAiSuggestion> rows = jdbcTemplate.query("""
                SELECT id, content, created_at, updated_at
                FROM global_ai_suggestions
                WHERE id = ? AND owner_user_id = ? AND is_deleted = 0
                LIMIT 1
                """, rowMapper, suggestionId, ownerUserId);
        return rows.isEmpty() ? Optional.empty() : Optional.of(rows.getFirst());
    }

    public GlobalAiSuggestion update(Long ownerUserId, Long suggestionId, String content, LocalDateTime now) {
        int changed = jdbcTemplate.update("""
                UPDATE global_ai_suggestions
                SET content = ?, updated_at = ?
                WHERE id = ? AND owner_user_id = ? AND is_deleted = 0
                """, normalizeContent(content), formatDateTime(now), suggestionId, ownerUserId);
        return changed > 0 ? findById(ownerUserId, suggestionId).orElse(null) : null;
    }

    public boolean softDelete(Long ownerUserId, Long suggestionId, LocalDateTime now) {
        return jdbcTemplate.update("""
                UPDATE global_ai_suggestions
                SET is_deleted = 1, deleted_at = ?, updated_at = ?
                WHERE id = ? AND owner_user_id = ? AND is_deleted = 0
                """, formatDateTime(now), formatDateTime(now), suggestionId, ownerUserId) > 0;
    }
